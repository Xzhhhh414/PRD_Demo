...
- 用正文**加粗**替代标题 4（`####`），避免层级过深
- 默认使用**中文**撰写

### Markdown 空行规则

文档需要在飞书文档中阅读，为了避免渲染出多余的留白，遵循以下空行规则：
- **表格上方**空一行、**表格下方**空一行（让表格前后有呼吸感）
- **其他所有位置不留空行**（标题前后、段落间、引用前后、列表前后、分隔线前后）
...
- **避免跨文档重复** — 通用逻辑（如分享行为）应写在实际使用该功能的模块段落中，而非作为独立的全局章节
...
> **「全局约定」段落不是必需的。** 只有当确实存在跨所有模块的通用规则（如统一的加载态、空状态处理、埋点规范等）时才写。能归入具体模块的规则（如登录态、货币体系）应放在对应模块中，不要为了填充模板而强行写全局约定。
